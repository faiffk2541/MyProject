...
public class RegisterJpaController implements Serializable {
    ...
    public void create(Register register) throws PreexistingEntityException, RollbackFailureException, Exception {
        ...
        try {
            utx.begin();
            em = getEntityManager();
            Customers customernumber = register.getCustomernumber();
            if (customernumber != null) {
                customernumber = em.getReference(customernumber.getClass(), customernumber.getCustomernumber());
                register.setCustomernumber(customernumber);
            }
            em.persist(register);
            if (customernumber != null) {
                customernumber.getRegisterList().add(register);
    ...
    public void edit(Register register) throws NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            utx.begin();
            em = getEntityManager();
            Register persistentRegister = em.find(Register.class, register.getEmail());
            Customers customernumberOld = persistentRegister.getCustomernumber();
            Customers customernumberNew = register.getCustomernumber();
    ...
    public void destroy(String id) throws NonexistentEntityException, RollbackFailureException, Exception {
        ...
        try {
            ...
            Customers customernumber = register.getCustomernumber();
            if (customernumber != null) {
                customernumber.getRegisterList().remove(register);
                customernumber = em.merge(customernumber);
            }
            em.remove(register);
            utx.commit();
